% 구 폴더 목록
% guNames = {'남구', '달서구', '동구', '북구', '서구', '수성구', '중구','군위군'};
guNames = {'구미','칠곡'};
...
for i = 1:length(guNames)
    % Waitbar 업데이트
    waitbar(i / length(guNames), hWaitbar, sprintf('Processing %s...', guNames{i}));
...
%     dilatedBoundary = imdilate(inBoundary, strel('disk', 2));
...
% 
%     % 경사도 11도 이하 및 초과인 지점의 좌표와 경사도 값 추출
%     validLat = latMesh(dilatedBoundary);
%     validLon = lonMesh(dilatedBoundary);
%     validSlope = slope(dilatedBoundary);
%     slopeCondition = mask(dilatedBoundary); % 경사도 11도 이하 = 1, 11도 초과 = 0
% 
%     % 데이터 테이블로 저장
%     T = table(validLat, validLon, validSlope, slopeCondition, 'VariableNames', {'Latitude', 'Longitude', 'Slope', 'Condition'});
%     writetable(T, fullfile('slope2', [guName, '_slope_with_conditions.csv']));
    
    % 경계선 정보를 저장
    boundaryData = table(x(k), y(k), 'VariableNames', {'Longitude', 'Latitude'});
    writetable(boundaryData, fullfile('boundary', [guName, '_boundary.csv']));
