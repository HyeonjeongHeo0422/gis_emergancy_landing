            ...
            if cluster_labels:
                ...
                if cluster_id != -1:
                    ...
                    if distance_to_uam < cluster_distances[cluster_id]:
                        ...
                        cluster_safe_centers[cluster_id] = safe_center
                    
                    # 클러스터별 폴리곤 개수 증가
                    cluster_counts[cluster_id] += 1

    return cluster_areas, cluster_distances, cluster_counts, cluster_safe_centers


# def calculate_cluster_weights(cluster_areas, cluster_distances):
#     """
#     클러스터별 가중치를 계산하는 함수.
...
#     # 모든 클러스터의 면적 합과 최소 거리를 하나의 배열로 결합
...
#         # 면적과 거리에 대한 가중치 합산
#         # weighted_score = 0.7 * normalized_area + 0.3 * (1 - normalized_distance)
    ...
    for region_name in region_names:
        ...
        for i in range(30):  # 30번 반복
            ...
            if filtered_data.empty:
                print("No data in filtered polygons. Adjust your filtering criteria.")
            ...
            if vertices_array.size != 0:
                ...
                # 7. 클러스터별 가중치 계산
                # 클러스터별 면적, 거리 저장
                cluster_areas, cluster_distances, cluster_counts, cluster_safe_center = assign_cluster_properties_new(filtered_data, clusters, vertices_array, uam_location)
        ...
        # 결과 저장
        results_df.to_csv('results/clustering/region_cluster_analysis.csv', index=False)
