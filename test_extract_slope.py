...
#                 color='blue',
#                 markersize=5,
#                 label='Elevation Points'
#             )
    
#     ax.set_title('Elevation Points within Selected Polygons')
#     ax.set_xlabel('Longitude')
#     ax.set_ylabel('Latitude')
#     plt.show()
...
if __name__ == "__main__":
    ...
    # 1. GIS 데이터 로드
    exclusion_data, boundary_gdf = load_data(exclusion_files, forest_file, soil_file, boundary_file)

    # 2. GIS 데이터 전처리
    polygons, boundary_gdf, exclusions_in_boundary, landing_able_sites = preprocess_data(exclusion_data, boundary_gdf)

    # 3. 비상착륙 후보지 선정(반지름이 20m인 원이 폴리곤 내부에 그려질 수 있는 곳)
    radius = 20
    landing_candidates = filter_landing_zones(polygons, radius)

    # 4. 결과 시각화
    # fig1, fig2 = visualize_results(boundary_gdf, landing_candidates, landing_able_sites, exclusions_in_boundary)

    # 4. 각 폴리곤에 고도 및 경사도 추가
    polygons_with_elevation = add_elevation_to_polygons(landing_candidates['Polygon'], elevation_points, elevation_contours)

    # (Debug) 경사도 데이터 출력
    # print_polygons_and_slopes(polygons_with_elevation)

    # 6. 폴리곤의 경사도 시각화
    fig3 = visualize_slope(boundary_gdf, polygons_with_elevation)

    # 안전 경사도를 가진 폴리곤 추출
    safe_polygons = extract_safe_slopes(polygons_with_elevation)
    # 안전 경사도 폴리곤 시각화
    fig5 = visualize_safe_polygons(boundary_gdf, safe_polygons)

    # 안전 경사도와 위험 경사도를 시각화
    fig4 = visualize_slope_with_risk(boundary_gdf, polygons_with_elevation)

    # # 안전 경사도를 가진 폴리곤 정보 추출
    # safe_polygons_df = pd.DataFrame({'Polygon': safe_polygons})
    # # 결과를 CSV로 저장
    # safe_polygons_df.to_csv(f'results/{region_name}/{region_name}_safe_polygons_info.csv', index=False)

    # 안전 경사도를 가진 폴리곤 정보 추출
    safe_threshold = 10
    safe_polygons_df = polygons_with_elevation[
        polygons_with_elevation['Slopes'].apply(lambda slopes: np.mean(slopes) <= safe_threshold)
    ]

    # 필요한 정보를 포함한 DataFrame 생성
    safe_polygons_info_df = pd.DataFrame({
        'Polygon': safe_polygons_df['Polygon'],
        'Area (m^2)': safe_polygons_df['Area (m^2)'],
        'Mean Elevation': safe_polygons_df['Mean Elevation'],
        'Max Elevation': safe_polygons_df['Max Elevation']
    })

    # 결과를 CSV로 저장
    # safe_polygons_info_df.to_csv(f'results/{region_name}/{region_name}_safe_polygons_info.csv', index=False)


    # 5. Filtered GIS data 저장
    # 경사도 고려
    # safe_polygons_df = pd.DataFrame({'Polygon': safe_polygons})
    # safe_polygons_df.to_csv(f'results/{region_name}/{region_name}_safe_polygons.csv', index=False)
    
    # save_results(landing_candidates, f'results/{region_name}/{region_name}_landing_candidates_feat_내부원_수계제외.csv')

    # 6. 시각화 결과 저장
    # save_visualization(fig1, f'results/{region_name}/면적 고려/{region_name}_비상착륙지_후보군_feat_내부원_수계제외.png')
    # save_visualization(fig2, f'results/{region_name}/추출/{region_name}_추출_feat_내부원_수계제외.png')
    
    # 경사도 고려
    # save_visualization(fig3, f'results/{region_name}/{region_name}_slope.png')
    # save_visualization(fig4, f'results/{region_name}/{region_name}_slope_risk_visualization.png')
    # save_visualization(fig5, f'results/{region_name}/{region_name}_safe_slope_visualization.png')

    # 고도 점 데이터 시각화
    visualize_elevation_points_within_polygons(polygons_with_elevation, elevation_points, boundary_gdf)
