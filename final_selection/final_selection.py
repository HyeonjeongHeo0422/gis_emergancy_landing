...
def main():
    ...
    output_file = "results/select_landing_location/final_landing_locations.csv"  # 저장할 파일 경로
    ...
    for _, row in tqdm(cluster_data.iterrows(), total=len(cluster_data), desc="Processing"):
        ...
        if is_landing_point_valid(optimal_safe_center, obstacles_gdf):
            ...
            if vertices_array.size != 0:
                ...
                end_time = time.time()
                print_color('clustefing time: ', color="yellow")
                ...
                end_time = time.time()
                print_color('one cluster selection time: ', color="yellow")
                print_color(end_time-start_time, color="yellow")
                ...
                if optimal_cluster_id is not None:    
                    ...
                    end_time = time.time()
                    print_color('landing location selection time: ', color="yellow")
                    print_color(end_time-start_time, color="yellow")

        # 결과 저장
        # save_final_landing_location(output_file, uam_location, final_landing_location)
