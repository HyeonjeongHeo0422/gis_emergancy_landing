...
"""
edge_order=1
수치 미분을 계산할 때 간단한 차분을 사용합니다. 구체적으로는 첫 번째 차이(First-order differences)를 사용하여, 인접한 데이터 포인트 간의 차이만을 고려.
이 방식은 계산이 빠르고 간단하지만, 경계 부근에서 미분값이 덜 정확할 수 있다.
"""
...
plt.figure(figsize=(10, 6))
plt.title('Terrain Slope Visualization')
plt.imshow(slope_percent, extent=extent, cmap='terrain', origin='lower')
plt.xlabel('Longitude')
plt.ylabel('Latitude')
plt.colorbar(label='Slope (%)')
plt.show()